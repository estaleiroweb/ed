...
final class IPC_MessagesQueue extends IPC {
	...
	protected function newKey() { return $this->resource=msg_get_queue($this->key, self::$perms); }
	protected function delKey() { if($this->resource) return msg_remove_queue($this->resource); }
	...
	public function set(array $data=array()){ if($this->resource) return msg_set_queue($this->resource,$data); }
}