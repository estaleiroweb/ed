 ...
 * 	protected $readonly=array('time'=>1,'laps'=>3);
 * 	function run() {
 * 		$this->debug();
...
abstract class Threads implements Iterator, Countable, ArrayAccess {
	...
	protected $statusList=array('not started','running','finished','paused','waiting','starting');
	protected $id=null;
	protected $pid=null;
	...
	final public function __construct(){
		...
		$this->initMain();
		$args=func_get_args();
		foreach($this->readonly as $k=>$v) if($args) $this->__set($k,array_shift($args)); else break;
		$this->parameters=$args;
		$this->setStart(false);
	}
	final public function __get($name)        { return @$this->readonly[$name]; }
	final public function __set($name,$value) { return $this->readonly[$name]=$value; }
	...
	final private function recursive($fn,$args,$out=[]){         //Excute the method $fn in every $this->data elements
		$this->debug();
		foreach($this->data as $obj) $out[]=call_user_func_array(array($obj,$fn),$args);
		return $out;
	}
	...
	final public function offsetSet($index, $value){                  // implements by ArrayAccess
		...
		if(is_object($value) && $value instanceof Threads) {
			...
			$this->data[$index]=$this->newClass($value);
		}
	...
	final public  function isPaused()                { return (bool)$this->getShared('paused'); }
	final public  function isWaiting()               { return (bool)$this->getShared('waitting'); }
	final public  function isFinished()              { return (bool)$this->getShared('finished'); }
	final public  function isTerminated()            { return $this->isFinished(); }
	final public  function isStarted()               { return (bool)$this->getShared('started'); }
	final public  function getPid()                  { return $this->pid?$this->pid:$this->pid=$this->getShared('pid'); }
	final public  function checkPid()                { return ($pid=$this->getPid())?trim(`ps -p {$pid} --no-heading -o pid`):false; }
	final private function setPid($pid=0)            { return $this->setShared('pid',$this->pid=$pid); }
	final private function setPause($value=false)    { return $this->setShared('paused',$value); }
	final private function setWait($value=false)     { return $this->setShared('waitting',$value); }
	final private function setFinished($value=false) { return $this->setShared('finished',$value); }
