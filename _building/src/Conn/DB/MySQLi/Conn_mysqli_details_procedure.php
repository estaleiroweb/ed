<?php
class Conn_mysqli_details_procedure extends Conn_mysqli_details_main
{
    ...
    protected function getStatus()
    {
        ...
        if ($this->version < 5) {
            $view = array();
            $this->setError("Database version doesn't support Procedures");
        }
        if ($obj === false) {
            $obj = array();
            $line = $this->conn->fastLine("SELECT * FROM information_schema.`ROUTINES` WHERE `ROUTINE_TYPE`='PROCEDURE' AND `ROUTINE_SCHEMA`='{$this->conn->escape_string($this->readonly['db'])}' AND `ROUTINE_NAME`='{$this->conn->escape_string($this->readonly['name'])}'");
            if ($line) {
                $obj = line;
                $obj['dtUpdate'] = $line['LAST_ALTERED'];
            } else
                $obj = $this->setError("Procedure doesn't exist");
            /*
