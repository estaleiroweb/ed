...
abstract class IPC {
	...
	final public function key() { return $this->key; }
	final public function resource() { return $this->resource; }
	...
	final public function keys($param=null){ //key, id => { values }
		...
		array_shift($cols);array_shift($cols);
	...
	final public function createKey_auto() {
		...
		switch($param){
			case '-S': $p=$param.' '.self::$max_acquire.' -p'.$perm; break;
			case '-M': $p=$param.' '.self::$size.' -p'.$perm; break;
			case '-Q': $p=$param; break;
			default: exit;
		}
