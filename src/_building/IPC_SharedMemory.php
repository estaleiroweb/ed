...
final class IPC_SharedMemory extends IPC {
	protected $param='-m';
	protected $data=array('names'=>array(),'pids'=>array(),);
	...
	public function __set($name,$value){ if($this->resource) return shm_put_var   ($this->resource, $this->tr($name), $value); }
	public function __isset($name)     { if($this->resource) return shm_has_var   ($this->resource, $this->tr($name)); }
	...
	private function set(){ if($this->resource) return shm_put_var($this->resource, 0, $this->data); }
