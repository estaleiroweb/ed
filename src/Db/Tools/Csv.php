...
class Csv {
	...
	protected $fnQuote;
	private $printHead,$csv;
	
	public function __construct($delimiter=self::TAB,$terminator=self::LF,$quote=1){
		...
		{$this->protect=array(
			'delimiter'=>$delimiter,
			'terminator'=>$terminator,/*CR,LF,CRLF,LFCR*/
			'quote'=>null,
			'head'=>true,//imprimir cabeçalho ou não
			'append'=>true,//true=adicionar ao final, false=cria novo
			'nullValue'=>'\\N',//'NULL',
			'encoding'=>null,
	...
	public function csv2tbl($tbl=null,$content=null){
		...
		if($tbl) {
			$conn=$this->conn;
			while($l=$this->getLine($line,$tr)) {
				if($this->protect['onheader']) $l=call_user_func($this->protect['onheader'],$l,$this);
				$arr[]=$l;
				$conn->save($tbl,$l,null,true);
			}
			$conn->save($tbl);
		}
		else while($l=$this->getLine($line,$tr)) {
			if($this->protect['onheader']) $l=call_user_func($this->protect['onheader'],$l,$this);
			$arr[]=$l;
		}
	...
	protected function getLine($line=array(),$tr=array()){
		if(!$this->csv) return false;
		$tam=strlen($this->protect['terminator']);
		//$d=preg_quote($this->protect['delimiter'],'/');
		//$n=preg_quote($this->protect['nullValue'],'/');
		//$lf=preg_quote($this->protect['terminator'],'/');
		//$end="({$this->protect['delimiter']}|{$this->protect['terminator']})/m";
		$end="({$this->protect['delimiter']}|[\r\n]+)/m";
		$aRegExp=array(
			'/^(?:"()"|'.preg_quote($this->protect['nullValue'],'/').')?'.$end, //null
			'/^"([^"]+)"'.$end, //quote
			//'/^([^'.$this->protect['delimiter'].']+)'.$end, //unquote
			'/^([^\r\n'.$this->protect['delimiter'].']+)'.$end, //unquote
		);
		$k=0;
		while($this->csv){
			if(preg_match('/^[\r\n]+/',$this->csv,$ret)){
				$this->csv=substr($this->csv,strlen($ret[0]));
				break;
			}
			//if(substr($this->csv,0,$tam)==$this->protect['terminator']) {$this->csv=substr($this->csv,$tam);break;}
			$ret=null;
			foreach($aRegExp as $er) if(preg_match($er,$this->csv,$ret)) break;
			if($ret) {
				$val=@$ret[1]==''?null:$ret[1];
				$this->csv=substr($this->csv,strlen($ret[0]));
			}
			else {
				$val=preg_replace('/\s+$/','',$this->csv);
				$this->csv=null;
				$ret[2]=$this->protect['terminator'];
			}
		...
		foreach($tr as $k=>$fld) $line[$fld]=null;
		return $line;
	}
	public function sql2csv($sql,$file=null){
		if(!preg_match('/^\s*select\s+/i',$sql)) return;
		$out='';
		$this->printHead=$this->head;
		$flag=$this->append?FILE_APPEND:0;
		if($this->encoding) mb_internal_encoding($this->encoding);
		
		$conn=$this->conn;
		$res=$conn->query($sql);
		while($line=$res->fetch_assoc()) $this->toCsv_line($line,$file,$out,$flag);
		$res->close();
		
		return $out;
	}
	public function array2csv(array $arr,$file=null){
		$out='';
		$this->printHead=$this->head;
		$flag=$this->append?FILE_APPEND:0;
		if($this->encoding) mb_internal_encoding($this->encoding);
		
		foreach($arr as $line) $this->toCsv_line($line,$file,$out,$flag);

		return $out;
	}
	
	protected function toCsv_line($line,$file,&$out='',&$flag=0){
		if($this->printHead) {
			$this->printHead=false;
			$this->toCsv_line(array_keys($line),$file,$out,$flag);
		}
		foreach($line as $k=>$v) $line[$k]=call_user_func($this->fnQuote,$v);
		$content=implode($this->delimiter,$line).$this->terminator;

		if(is_null($file)) $out.=$content;
		elseif($file && is_string($file)) file_put_contents($file,$content,$flag);
		$flag=FILE_APPEND;
	}
	
	protected function fieldQuote_none($value){
		if(is_null($value)) return $this->nullValue;
		if(is_numeric($value)) return $value;
		return $this->escape($value);
	}
	protected function fieldQuote_normal($value){
		if(is_null($value)) return $this->nullValue;
		if(is_numeric($value)) return $value;
		return '"'.$this->escape($value).'"';
	}
	protected function fieldQuote_smart($value){
		if(is_null($value)) return $this->nullValue;
		if(is_numeric($value)) return $value;
		$esc=$this->escape($value);
		return strpos($value,$this->protected['delimiter'])===false?$esc:'"'.$esc.'"';
	}
	protected function fieldQuote_full($value){
		if(is_null($value)) return $this->nullValue;
		if(is_numeric($value)) return '"'.$value.'"';
		return '"'.$this->escape($value).'"';
	}
